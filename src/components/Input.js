...
import React, { useState } from 'react'; /* STEP 0 DONE */
...
export default function Input() {
  ...
  const style = {
    fontSize: '1.5em',
    marginBottom: '0.3em',
    color: (inputValue.length > 10? 'royalblue': 'crimson') /* STEP 2 DONE*/
  };

  return (
    <div className='widget-input container'>
      <h2>Input</h2>
      <div id='output' style={style}>{inputValue.toUpperCase}</div> {/* STEP 3 DONE*/}
      <div>
        <input id='input' type='text' onChange={changeInput} value={inputValue}/> {/* STEP 6 DONE*/}
